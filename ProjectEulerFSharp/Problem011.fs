...
let getCell x y (cells:int[][]) =
  match (x,y) with
  | _,y when y > cells.Length-1 || y < 0 -> 0 
  | x,_ when x > cells.[0].Length-1 || x < 0 -> 0 
  | x,y -> cells.[y].[x]

let getCells x y numberToGet translator cells =
  seq{0..numberToGet-1}
  |> Seq.map (fun i -> 
    let (tx, ty) = translator x y i
    getCell tx ty cells
  )
  |> Seq.toArray
...
let ``get cell in cells``()=
  ...
  cells |> getCell 0 0 |> should equal 1
  cells |> getCell 1 0 |> should equal 2
  cells |> getCell 2 0 |> should equal 3
  cells |> getCell 0 1 |> should equal 4
...
let ``get cell not in cells``()=
  ...
  cells |> getCell 0 0 |> should equal 1
  cells |> getCell 0 1 |> should equal 0
...
let ``get cells in cells``()=
  ...
  cells |> getCells 0 0 2 (fun x y i -> (x+i,y)) |> should equal [|1;2|]
